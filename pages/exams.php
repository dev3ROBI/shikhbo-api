...
if (in_array($action, ['add_exam','edit_exam'])) {
    ...
    // exam_date: খালি থাকলে null, নাহলে সঠিক datetime
...
<?php if (isset($error)): ?>
<div
    class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4"><?php echo sanitizeOutput($error); ?></div><?php endif; ?>
<?php if (isset($success)): ?>
<div
    class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-4"><?php echo sanitizeOutput($success); ?></div><?php endif; ?>
...
<div
    ...
    <div>
        ...
        <p class="text-gray-500 mt-1"><?php echo $totalExams; ?>
            exams total</p>
    </div>
...
<form method="GET" class="mb-6 flex flex-wrap gap-2 items-center">
    ...
    <select
        ...
        <option
            value="completed"
            <?php echo $statusFilter==='completed'?'selected':''; ?>>Completed</option>
    </select>
    <button
        type="submit"
        class="px-4 py-2 bg-shikhbo-primary text-white rounded-lg text-sm">Filter</button>
    <?php if ($search||$statusFilter): ?>
    <a
        href="index.php?page=exams"
        class="px-4 py-2 border border-gray-300 rounded-lg text-sm">Clear</a><?php endif; ?>
</form>
...
<div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3">
                    ...
                    <p class="text-xs text-gray-400 truncate max-w-xs"><?php echo sanitizeOutput(substr($exam['description'],0,60)); ?></p><?php endif; ?>
                </td>
                ...
                <td class="px-4 py-3 text-sm"><?php echo $exam['duration_minutes']; ?>
                    min</td>
                <td class="px-4 py-3 text-sm font-medium"><?php echo $exam['total_marks']; ?></td>
                <td class="px-4 py-3">
                    <span
                        class="px-2 py-1 text-xs font-semibold rounded-full <?php echo $exam['status']==='active'?'text-green-800 bg-green-100':($exam['status']==='completed'?'text-red-800 bg-red-100':'text-yellow-800 bg-yellow-100'); ?>"><?php echo ucfirst($exam['status']); ?></span>
                </td>
                <td class="px-4 py-3 text-sm">
                    <div class="flex items-center space-x-2">
                        <a
                            href="index.php?page=exam_attempt&exam_id=<?php echo $exam['id']; ?>"
                            class="text-green-600 hover:underline"
                            title="Attempt">
                            <i class="fa-solid fa-play"></i>
                        </a>
                        <a
                            href="index.php?page=questions&exam_id=<?php echo $exam['id']; ?>"
                            class="text-blue-600 hover:underline"
                            title="Questions">
                            <i class="fa-solid fa-list-check"></i>
                        </a>
                        <button
                            onclick="editExam(<?php echo htmlspecialchars(json_encode($exam), ENT_QUOTES, 'UTF-8'); ?>)"
        ...
        <?php else: ?>
            <tr>
                <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                    <i class="fa-solid fa-file-circle-exclamation text-3xl mb-2 block"></i>No exams yet.</td>
            </tr>
            <?php endif; ?>
        </tbody>
    </table>
    ...
    <div class="px-6 py-3 border-t flex justify-between text-sm">
        ...
        <div class="flex space-x-1">
            ...
            <a
                href="index.php?page=exams&p=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&status=<?php echo $statusFilter; ?>"
                class="px-3 py-1 border rounded <?php echo $i===$page_num?'bg-shikhbo-primary text-white':'hover:bg-gray-100'; ?>"><?php echo $i; ?></a>
            <?php endfor; ?>
        </div>
    </div>
    <?php endif; ?>
...
<div
    ...
    <div
        class="relative bg-white rounded-xl shadow-xl w-full max-w-xl mx-4 p-6 max-h-screen overflow-y-auto">
...
<script>
    function openExamModal(exam = null) {
        ...
        if (exam) {
            document
                .getElementById('examModalTitle')
                .textContent = 'Edit Exam';
            document
                .getElementById('examAction')
                .value = 'edit_exam';
            document
            ...
            document
                ...
                .value = exam.exam_date
                    ...
                    : '';
            document
                .getElementById('examDesc')
                .value = exam.description || '';
        } else {
            document
                .getElementById('examModalTitle')
                .textContent = 'Create Exam';
            document
                .getElementById('examAction')
                .value = 'add_exam';
            document
    ...
    function deleteExam(id) {
        if (confirm('Delete this exam? This will also delete all associated questions.')) {
            document
                .getElementById('deleteExamId')
                .value = id;
            document
                .getElementById('deleteExamForm')
                .submit();
        }
    }
</script>