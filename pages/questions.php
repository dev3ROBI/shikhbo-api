...
function renderCategoryTreeForFilter($tree, $selectedExamId, $level = 0) {
    ...
    foreach ($tree as $cat) {
        ...
        $html .= "<div class='flex items-center py-2 px-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors group' onclick='toggleCatNode(this, {$cat['id']})'>";
        $html .= "<span class='w-5 h-5 flex items-center justify-center mr-2 text-gray-400 dark:text-gray-500'>";
        ...
        $html .= "<span class='text-sm text-gray-700 dark:text-gray-200 flex-1'>" . sanitizeOutput($cat['name']) . "</span>";
        $html .= "<span class='text-xs text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full'>{$cat['exam_count']}</span>";
        $html .= "</div>";

        // Load exams for this category (AJAX loaded)
        $html .= "<div class='cat-exams hidden ml-7' id='cat-exams-{$cat['id']}'>";
        $html .= "<div class='text-xs text-gray-400 dark:text-gray-500 py-1'>Loading exams...</div>";
        $html .= "</div>";
...
<?php if (isset($error)): ?><div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-lg mb-4"><?php echo sanitizeOutput($error); ?></div><?php endif; ?>
<?php if (isset($success)): ?><div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-300 px-4 py-3 rounded-lg mb-4"><?php echo sanitizeOutput($success); ?></div><?php endif; ?>

<!-- Header -->
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Question Bank</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1"><?php echo $total; ?> questions total<?php echo $selectedExamTitle ? ' • <span class="text-shikhbo-primary font-medium">' . sanitizeOutput($selectedExamTitle) . '</span>' : ''; ?></p>
    </div>
    <button onclick="openQuestionModal()" class="px-4 py-2 bg-shikhbo-primary text-white rounded-lg text-sm hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200 dark:shadow-indigo-900/30">
...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md dark:shadow-gray-900/20 p-4 sticky top-4" style="max-height: calc(100vh - 120px); overflow-y: auto;">
            <div class="flex items-center justify-between mb-3">
                <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider"><i class="fa-solid fa-layer-group mr-2 text-gray-400 dark:text-gray-500"></i>Exam Filter</h3>
                <?php if ($examFilter): ?>
            ...
            <div class="relative mb-3">
                <input type="text" id="treeSearch" placeholder="Filter categories..." 
                       class="w-full pl-8 pr-3 py-1.5 text-xs border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg focus:ring-2 focus:ring-shikhbo-primary focus:border-transparent outline-none">
                <i class="fa-solid fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 text-xs"></i>
            </div>
            ...
            <div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                <label class="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Or select exam:</label>
                <form method="GET" action="index.php" id="quickExamForm">
                    <input type="hidden" name="page" value="questions">
                    <select name="exam_id" onchange="this.form.submit()" class="w-full px-2 py-1.5 text-xs border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg">
    ...
    <div class="lg:col-span-3">
        ...
        <form method="GET" class="mb-4 flex gap-2">
            ...
            <input type="text" name="search" value="<?php echo sanitizeOutput($search); ?>" placeholder="Search questions..." class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-shikhbo-primary outline-none">
            ...
            <?php if ($search): ?><a href="index.php?page=questions<?php echo $examFilter?'&exam_id='.$examFilter:''; ?>" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Clear</a><?php endif; ?>
        </form>
        ...
        <div class="space-y-4">
            <?php if ($questions->num_rows > 0): ?>
                <?php $qNum = $offset + 1; while ($q = $questions->fetch_assoc()): ?>
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-5 hover:shadow-md transition-all question-card">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-xs text-gray-400 dark:text-gray-500 font-mono bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded">Q<?php echo $qNum++; ?></span>
                                    <span class="text-xs text-gray-400 dark:text-gray-500"><?php echo sanitizeOutput($q['exam_title'] ?? 'Unassigned'); ?></span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full"><?php echo $q['marks']; ?> mark<?php echo $q['marks']>1?'s':''; ?></span>
                                </div>
                                <p class="text-gray-800 dark:text-gray-100 font-medium mt-2"><?php echo sanitizeOutput($q['question_text']); ?></p>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                            <?php foreach(['a','b','c','d'] as $opt): ?>
                                <div class="text-sm px-3 py-2 rounded-lg <?php echo strtolower($q['correct_answer'])===$opt ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-300 font-medium' : 'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300'; ?>">
                                    <span class="font-semibold mr-1"><?php echo strtoupper($opt); ?>.</span><?php echo sanitizeOutput($q['option_'.$opt]); ?>
                                    <?php if (strtolower($q['correct_answer'])===$opt): ?><i class="fa-solid fa-check text-green-600 dark:text-green-400 ml-2"></i><?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
        ...
        <?php if ($totalPages > 1): ?>
            <div class="mt-6 flex justify-center space-x-1">
                <?php for ($i=1;$i<=$totalPages;$i++): ?>
                    <a href="index.php?page=questions&p=<?php echo $i; ?>&exam_id=<?php echo $examFilter; ?>&search=<?php echo urlencode($search); ?>" class="px-3 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded <?php echo $i===$page_num?'bg-shikhbo-primary text-white border-shikhbo-primary':'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'; ?>"><?php echo $i; ?></a>
                <?php endfor; ?>
            </div>
        <?php endif; ?>
    </div>
...
<!-- Add/Edit Question Modal (same structure as before with dark classes) -->
<div id="questionModal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 max-h-screen overflow-y-auto">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100" id="qModalTitle">Add Question</h3>
            <button onclick="closeQuestionModal()" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300"><i class="fa-solid fa-xmark text-xl"></i></button>
        </div>
        <form method="POST" id="questionForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="exam_id" id="qExam" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="marks" id="qMarks" value="1" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm">
                </div>
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Question</label>
                    <textarea name="question_text" id="qText" rows="3" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm"></textarea>
                </div>
                <?php foreach(['a','b','c','d'] as $opt): ?>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Option <?php echo strtoupper($opt); ?></label>
                        <input type="text" name="option_<?php echo $opt; ?>" id="qOpt<?php echo strtoupper($opt); ?>" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm">
                    </div>
                <?php endforeach; ?>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Correct Answer</label>
                    <select name="correct_answer" id="qCorrect" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm">
                        <option value="a">A</option><option value="b">B</option><option value="c">C</option><option value="d">D</option>
                    </select>
                </div>
            </div>
            <div class="mt-6 flex justify-end space-x-3">
                <button type="button" onclick="closeQuestionModal()" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300">Cancel</button>
...
function toggleCatNode(header, catId) {
    ...
    // Load exams for this category via AJAX with direct=1
            ...
            .then(data => {
                if (data.status === 'success' && data.exams.length > 0) {
                    examsDiv.innerHTML = data.exams.map(e => `
                        <a href="index.php?page=questions&exam_id=${e.id}" class="block text-xs py-1.5 px-2 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/20 text-gray-600 dark:text-gray-300 hover:text-shikhbo-primary transition-colors">
                            <i class="fa-solid fa-file-alt mr-1 text-gray-400 dark:text-gray-500"></i>${e.title}
                        </a>
                    `).join('');
                } else {
                    examsDiv.innerHTML = '<div class="text-xs text-gray-400 dark:text-gray-500 py-1">No exams</div>';
                }
