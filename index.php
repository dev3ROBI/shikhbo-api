<?php
/**
 * Shikhbo Admin Panel — Enhanced Router
 * Notification Panel · Support Tickets · Fixed Icon Alignment · Mobile‑First
 */
...
if ($ticketsTableExists) {
    ...
    $mysqli->query("CREATE TABLE IF NOT EXISTS support_tickets (
        ...
        user_id INT UNSIGNED, subject VARCHAR(255), message TEXT,
...
<aside id="sidebar" class="sidebar fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 -translate-x-full">
    <div class="flex items-center justify-between h-16 px-5 border-b border-gray-200">
        <a href="index.php" class="flex items-center space-x-2 flex-shrink-0">
            ...
            <span class="text-xl font-bold text-gray-800 whitespace-nowrap">Shikhbo</span>
        </a>
        <button id="sidebarClose" class="lg:hidden text-gray-400 hover:text-gray-700 p-1 rounded-md hover:bg-gray-100"><i class="fa-solid fa-xmark text-xl"></i></button>
    </div>
                ...
                <li>
                    <a href="index.php?page=<?php echo $navPage; ?>"
                       class="nav-link flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200
                              <?php echo $isActive ? 'bg-shikhbo-primary text-white shadow-md shadow-indigo-200' : 'text-gray-600 hover:bg-gray-100 hover:text-shikhbo-primary'; ?>">
                        <span class="w-8 h-8 flex items-center justify-center rounded-lg flex-shrink-0
                                     <?php echo $isActive ? 'bg-white/20' : 'bg-gray-100'; ?> mr-3">
    ...
    <div class="p-4 border-t border-gray-200">
        <div class="flex items-center space-x-3">
            ...
            <div class="flex-1 min-w-0"><p class="text-sm font-medium text-gray-700 truncate"><?php echo sanitizeOutput($admin['name']); ?></p><p class="text-xs text-gray-400 truncate"><?php echo sanitizeOutput($admin['email']); ?></p></div>
        </div>
    </div>
</aside>
...
<div class="flex-1 flex flex-col overflow-hidden">
    <header class="bg-white shadow-sm border-b border-gray-200">
        <div class="flex items-center justify-between h-16 px-4 lg:px-8">
            <button id="sidebarToggle" class="lg:hidden text-gray-600 hover:text-gray-800 p-2 rounded-md hover:bg-gray-100 flex-shrink-0"><i class="fa-solid fa-bars text-xl"></i></button>
            <div class="hidden sm:flex items-center space-x-2 ml-2 min-w-0"><h2 class="text-lg font-semibold text-gray-700 truncate"><?php echo sanitizeOutput($pageTitle); ?></h2></div>
            ...
            <div class="flex items-center space-x-1 ml-auto">
                ...
                <div class="relative" id="notifDropdown">
                    <button id="notifButton" class="relative p-2 text-gray-500 hover:text-shikhbo-primary rounded-full hover:bg-gray-100 transition-colors">
                    ...
                    <div id="notifPanel" class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 hidden z-50 transform opacity-0 scale-95 transition-all duration-200 origin-top-right max-h-96 overflow-y-auto">
                        <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                            <h3 class="text-sm font-semibold text-gray-700"><i class="fa-solid fa-shield-halved text-red-500 mr-1"></i>Security Alerts</h3>
                        ...
                        <div class="divide-y divide-gray-50">
                            <?php if ($recentFailed && $recentFailed->num_rows > 0): ?>
                                <?php while ($nf = $recentFailed->fetch_assoc()): ?>
                                    <div class="px-4 py-2.5 hover:bg-gray-50 text-xs">
                                        <div class="flex items-start space-x-2">
                                            <i class="fa-solid fa-circle-exclamation text-red-400 mt-0.5 flex-shrink-0"></i>
                                            <div class="min-w-0"><p class="text-gray-700 font-medium truncate"><?php echo sanitizeOutput($nf['email']); ?></p><p class="text-gray-400">IP: <?php echo sanitizeOutput($nf['ip_address']); ?> · <?php echo date('H:i', strtotime($nf['attempt_time'])); ?></p></div>
                                        </div>
                                    </div>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <div class="px-4 py-6 text-center text-xs text-gray-400"><i class="fa-solid fa-circle-check text-green-400 text-lg block mb-1"></i>No alerts</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative" id="ticketDropdown">
                    <button id="ticketButton" class="relative p-2 text-gray-500 hover:text-shikhbo-primary rounded-full hover:bg-gray-100 transition-colors">
                    ...
                    <div id="ticketPanel" class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 hidden z-50 transform opacity-0 scale-95 transition-all duration-200 origin-top-right max-h-96 overflow-y-auto">
                        <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                            <h3 class="text-sm font-semibold text-gray-700"><i class="fa-solid fa-life-ring text-orange-500 mr-1"></i>Support Tickets</h3>
                        ...
                        <div class="divide-y divide-gray-50">
                            <?php if ($ticketsTableExists && $recentTickets && $recentTickets->num_rows > 0): ?>
                                <?php while ($tk = $recentTickets->fetch_assoc()): ?>
                                    <div class="px-4 py-2.5 hover:bg-gray-50 text-xs">
                                        <p class="text-gray-700 font-medium truncate"><?php echo sanitizeOutput($tk['subject']); ?></p>
                                        <p class="text-gray-400"><?php echo sanitizeOutput($tk['user_name'] ?? 'Unknown'); ?> · <span class="px-1.5 py-0.5 rounded-full text-[10px] <?php echo $tk['status']==='open'?'bg-orange-100 text-orange-700':'bg-green-100 text-green-700'; ?>"><?php echo ucfirst(str_replace('_',' ',$tk['status'])); ?></span></p>
                                    </div>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <div class="px-4 py-6 text-center text-xs text-gray-400"><i class="fa-solid fa-inbox text-lg block mb-1"></i>No tickets</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative" id="profileDropdown">
                    <button id="profileButton" class="flex items-center space-x-2 p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
                        <img src="https://ui-avatars.com/api/?name=<?php echo urlencode($admin['name']); ?>&background=4F46E5&color=fff&size=32" alt="" class="w-8 h-8 rounded-full flex-shrink-0">
                        <span class="text-sm font-medium text-gray-700 hidden md:block truncate max-w-[80px]"><?php echo sanitizeOutput($admin['name']); ?></span>
                        <i class="fa-solid fa-chevron-down text-xs text-gray-400 transition-transform duration-200 flex-shrink-0" id="dropdownChevron"></i>
                    </button>
                    <div id="profileMenu" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 hidden z-50 transform opacity-0 scale-95 transition-all duration-200 origin-top-right">
                        <div class="px-4 py-3 border-b border-gray-100"><p class="text-sm font-medium text-gray-800"><?php echo sanitizeOutput($admin['name']); ?></p><p class="text-xs text-gray-500 truncate"><?php echo sanitizeOutput($admin['email']); ?></p></div>
...
<div id="ticketModal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
    ...
    <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
        <div class="flex items-center justify-between mb-4"><h3 class="text-lg font-semibold">New Support Ticket</h3><button onclick="closeTicketModal()" class="text-gray-400 hover:text-gray-600"><i class="fa-solid fa-xmark text-xl"></i></button></div>
        <form id="ticketForm" class="space-y-4" onsubmit="submitTicket(event)">
            <?php echo getCSRFTokenField(); ?>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Subject</label><input type="text" id="ticketSubject" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-shikhbo-primary outline-none"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Message</label><textarea id="ticketMessage" rows="3" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-shikhbo-primary outline-none"></textarea></div>
            <div class="flex justify-end space-x-3"><button type="button" onclick="closeTicketModal()" class="px-4 py-2 border border-gray-300 rounded-lg text-sm">Cancel</button><button type="submit" class="px-4 py-2 bg-shikhbo-primary text-white rounded-lg text-sm">Submit</button></div>
        </form>
    </div>
</div>
