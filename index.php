...
$adminPages = [
    ...
    'exam_attempt', 'app_control', 'courses', 'transactions'
];

// Role-based page permissions
$rolePages = [
    'Administrator' => ['dashboard','students','admins','settings','exams','questions','results','categories','database','exam_attempt','app_control','courses','transactions'],
    'Moderator'     => ['dashboard','exams','questions','categories','exam_attempt','results','courses','transactions'],
...
if ($page && in_array($page, $adminPages)) {
    ...
    $navItems = [
        ...
        ['transactions', 'fa-receipt',       'Transactions'],
...
<div id="ticketModal" class="fixed inset-0 z-50 hidden">
    <div class="absolute inset-0 modal-backdrop" onclick="closeTicketModal()"></div>
    <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
    <div class="modal-content w-full max-w-md pointer-events-auto">
        <div class="modal-header flex items-center justify-between sticky top-0 z-10"><h3 class="text-lg font-semibold">New Support Ticket</h3><button onclick="closeTicketModal()" class="p-2 rounded-lg transition-all"><i class="fa-solid fa-xmark text-lg"></i></button></div>
        <form id="ticketForm" class="modal-body-scroll space-y-4" onsubmit="submitTicket(event)">
            ...
            <div class="modal-actions"><button type="button" onclick="closeTicketModal()" class="btn-cancel">Cancel</button><button type="submit" class="btn-save">Submit</button></div>
        </form>
    </div>
    </div>
</div>
